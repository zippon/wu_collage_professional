...
CollageProfessional::CollageProfessional(const std::string img_list_stress,
  ...
  stress_collage_ = new CollageAdvanced(img_alpha_stress_,
                                        canvas_width);
  regular_1_collage_ = NULL;
  regular_2_collage_ = NULL;
  return;
}
...
void CollageProfessional::ReadImageList(const std::string img_list_stress,
  ...
  while (!input_list_stress.eof()) {
    ...
    img_alpha_stress_.push_back(new_unit);
  }
  while (!input_list_regular.eof()) {
    ...
    new_unit.alpha_recip_ = static_cast<float>(img.rows) / img.cols;
    new_unit.image_path_ = img_path;
    img_alpha_regular_.push_back(new_unit);
  }
...
bool CollageProfessional::CreateCollageHV (float canvas_width, float canvas_height) {
  // Step 1: calculate the size of the stress collage.
  float stress_weight = stress_ratio_ * img_alpha_stress_.size();
  float regular_weight = img_alpha_regular_.size();
  ...
  float new_stress_width, new_stress_height;
  if (amplifier <= 1) {
    new_stress_width= stress_collage_->canvas_width() * amplifier;
    new_stress_height = stress_collage_->canvas_height() * amplifier;
  } else {
    new_stress_width = stress_collage_->canvas_width();
    new_stress_height = stress_collage_->canvas_height();
  }
  ...
  if ((regular_1_alpha != 0) && (regular_2_alpha != 0)) {
    std::vector<AlphaUnit> img_alpha_regular_1;
    std::vector<AlphaUnit> img_alpha_regular_2;
    for (int i = 0; i < img_alpha_regular_.size(); ++i) {
      if (i % 2) img_alpha_regular_1.push_back(img_alpha_regular_[i]);
      else img_alpha_regular_2.push_back(img_alpha_regular_[i]);
    }
    regular_1_collage_ = new CollageAdvanced(img_alpha_regular_1,
                                             regular_1_width);
    regular_2_collage_ = new CollageAdvanced(img_alpha_regular_2,
                                             regular_2_width);
  ...
  } else if (regular_1_alpha != 0) {
    regular_1_collage_ = new CollageAdvanced(img_alpha_regular_, regular_1_width);
    success = regular_1_collage_->CreateCollage(regular_1_alpha, 1.1);
    if (success == -1) {
      std::cout << "Failure in CreateCollage 4" << std::endl;
    }
  } else if (regular_2_alpha != 0) {
    regular_2_collage_ = new CollageAdvanced(img_alpha_regular_, regular_2_width);
    success = regular_2_collage_->CreateCollage(regular_2_alpha, 1.1);
    if (success == -1) {
      std::cout << "Failure in CreateCollage 5" << std::endl;
    }
  } else {
    std::cout << "Failure in CreateCollage regular collages" << std::endl;
    return false;
  }
  
  
  // Step 4: connect all the three generated collages.
  if (regular_1_collage_ && regular_2_collage_) {
  ...
  } else if (regular_1_collage_) {
    tree_root_->split_type_ = 'v';
    tree_root_->is_leaf_ = false;
    tree_root_->left_child_ = stress_collage_->tree_root_;
    stress_collage_->tree_root_->parent_ = tree_root_;
    stress_collage_->tree_root_->child_type_ = 'l';
    tree_root_->right_child_ = regular_1_collage_->tree_root_;
    regular_1_collage_->tree_root_->parent_ = tree_root_;
    regular_1_collage_->tree_root_->child_type_ = 'r';
  } else if (regular_2_collage_) {
    tree_root_->split_type_ = 'h';
    tree_root_->is_leaf_ = false;
    tree_root_->left_child_ = stress_collage_->tree_root_;
    stress_collage_->tree_root_->parent_ = tree_root_;
...
float CollageProfessional::CalculateC1 (float canvas_width, float canvas_height) {
  float t_1 = stress_ratio_ / (stress_ratio_ *
                               static_cast<int>(img_alpha_stress_.size() +
                               static_cast<int>(img_alpha_regular_.size())));
  ...
  if (regular_2_collage_ ) {
    for (int i = 0; i < regular_2_collage_->tree_leaves_.size(); ++i) {
      float tile_size = regular_2_collage_->tree_leaves_[i]->position_.height_ *
      regular_2_collage_->tree_leaves_[i]->position_.width_;
      float s_2 = tile_size / canvas_size;
      if ((s_2 / t_2) < 0.5) {
        c1 += 5 * (s_2 - t_2) * (s_2 - t_2);
      } else {
